...
class Vocabulary:
    ...
    def add_new_words(self, words: list, overwrite=True):
        ...
        to_add = {}
        for word in results:
            to_add[word['word']] = word

        existing_words = [word['word'] for word in self.words_modified]
        new_words = [value for key, value in to_add.items() if key not in existing_words]
    ...
    @staticmethod
    def anki_tags_add(file, folder='results'):
